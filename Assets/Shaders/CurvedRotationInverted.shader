...
Shader "Unlit/CurvedRotationInverted"
{ 
	Properties
	{
		_MainTex ("Texture", 2D) = "white" {}
	//	_Inverted("inverted", Range(0,1)) = 0
	}
	SubShader
	{
...
#include "UnityCG.cginc"
		...
		{
			// sample the texture
			fixed3 colorfixer = lerp( float3(1,0,0), 1, abs(_Inverted - 0.5) * 2);
			fixed4 col = tex2D(_MainTex, i.uv) * i.color;
			col = lerp(col, 1 - col, _Inverted);
			col.rgb *= colorfixer;
		// apply fog
